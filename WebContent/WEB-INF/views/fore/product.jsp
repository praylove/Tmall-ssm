...
<title>天猫tmall.com--商品</title>

<div class="g-mid">
	<a href="../category/${p.category.id}">
...
<div class="g-mid" style="height: 550px;">
	<div class="m-product-pic">
		<div id="fullpic" class="fullpic">
			<c:forEach items="${p.productSingleImages}" var="si" varStatus="status">
				...
				id="pic${status.count}" src="image/product/${p.id}/${si.id}.jpg" />
			</c:forEach>
		</div>
		<ul id="mpic">
			<c:forEach items="${p.productSingleImages}" var="si" varStatus="status">
				<li>
					<a href="javascript:;" rel="pic${status.count}"><img class="mpic" src="image/product/${p.id}/${si.id}.jpg" /></a>
				</li>
			</c:forEach>
		</ul>
	</div>
	<div class="m-essential">
		...
		<div class="price">
			<img src="image/site/gouwuquan.png" /><span> 全天猫实物商品通用</span>
			<div>
		...
		<div class="m-selected-cnt">
			...
			<span class="change-button">
				<a id="countUp" href="javascript:;" class="glyphicon glyphicon-chevron-up"></a>
				<a id="countDown" href="javascript:;" class="glyphicon glyphicon-chevron-down"></a>
			</span>
...
<div class="g-description">
	<div class="m-tabs">
		<a href="javascript:;" id="img_a" class="active">商品详情</a>
		<a href="javascript:;" id="review_a">累计评价 ${p.reviewCount}</a>
	</div>
	<div id="img" class="m-description-img">
		<div class="details">
			...
			<c:forEach items="${p.productDetailsImages}" var="di">
				<img src="image/product/${p.id}/${di.id}.jpg" width="790px" />
			</c:forEach>
		</div>
	</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#selectedCount").blur(function() {
            ...
            var s = $(this).val();
            if(!re.test(s))
            	console.log(s.match(/[1-9][0-9]*/));
                $(this).val(s.match(/[1-9][0-9]*/));
            if($(this).val() === "")
                $(this).val("1");
